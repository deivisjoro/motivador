<?php  // $Id: lib.php,v 1.137.2.57 2009/10/27 11:24:56 nicolasconnault Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
function data_print_template($template, $records, $data, $search='', $page=0, $return=false) {
    ...
    // Then we generate strings to replace for normal tags
        foreach ($fields as $field) {
            ...
            $patterns[]='[[calificacion]]';
            $calificaciones = data_get_ratings($record->id);
		    $contador = 0;
		    $acumulador = 0;
		    foreach($calificaciones as $c){
		   		$contador++;
		   		$acumulador = $acumulador + $c->rating;
		    }
		    
		    $promedio2 = ($acumulador / $contador);
		    
		    $promedio2 = number_format($promedio2, 1, ',', '');
		    
		    $promedio = ($acumulador / $contador)*2;
		    
		    $promedio = number_format($promedio, 1, '', ''); 
		    $promedio = '<img src="../../motivador/itemG.gif" height="10" width="'.$promedio.'"><img src="../../motivador/itemR.gif" height="10" width="'.(100-$promedio).'"> ('.$promedio2.')';
            $replacement[] = $promedio;
        }
...
function data_print_ratings($data, $record) {
    ...
    $cm = get_coursemodule_from_instance('data', $data->id);
    $context = get_context_instance(CONTEXT_MODULE, $cm->id);
    if ($data->assessed and !empty($USER->id) or data_isowner($record->id)) {
...
// Scale is an array of ratings
    ...
    $calificaciones = data_get_ratings($recordid);
    $contador = 0;
    $acumulador = 0;
    foreach($calificaciones as $c){
   		$contador++;
